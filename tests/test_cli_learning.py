...
def test_validate_and_normalize_commands_are_available_and_fix_missing_ids(
    ...
    assert (
        "missing_id" in invalid.stdout.lower()
        or "missing explicit recall:id" in invalid.stdout.lower()
    )

    normalized = runner.invoke(app, ["normalize", "--deck", "architecture", "--write"])
    assert normalized.exit_code == ExitCode.OK
    assert "architecture-was-ist-cqrs" in (decks_dir / "architecture.md").read_text(
        encoding="utf-8"
    )
...
def test_scan_next_review_stats_work_end_to_end_via_cli_json(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    decks_dir = tmp_path / "decks"
    ...
    scan_result = runner.invoke(
        app, ["scan", "--deck", "architecture", "--format", "json"]
    )
    ...
    next_result = runner.invoke(
        app, ["next", "--deck", "architecture", "--show-answer", "--format", "json"]
    )
    ...
    stats_result = runner.invoke(
        app, ["stats", "--deck", "architecture", "--format", "json"]
    )
...
def test_next_returns_no_due_cards_exit_code_when_all_cards_are_in_future(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.chdir(tmp_path)
    decks_dir = tmp_path / "decks"
